...
#access CouchDB using Tornado's httpclient. The source code is released to the
#public domain as free software. There is no issue tracker, so please send
#comments and feedback to: jacob@nephics.com
#
    ...
    def _view(self, url, raise_error=True, **kwargs):
        ...
        if kwargs:
            for key, value in kwargs.iteritems():
                if key == 'keys':
                    body = self._json_encode({'keys': value})
                else:
                    value = url_escape(self._json_encode(value))
